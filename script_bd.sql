--------------------------------------------------------------------------------
-- KuraVet - script_bd.sql
-- Sprint 3 - DevOps Tools & Cloud Computing
--
-- DDL completo e carga inicial do Banco de Dados SQL do Azure (PaaS).
-- Consolidacao das migrations Flyway de
-- src/main/resources/db/migration/sqlserver, entregue como artefato
-- unico conforme exigido pela rubrica.
--
-- ATENCAO: em execucao normal NAO e preciso rodar este script a mao.
-- O Flyway aplica as mesmas migrations automaticamente no start da
-- aplicacao. Ele existe para leitura, conferencia e recriacao manual
-- (por exemplo no SSMS ou no Azure Data Studio).
--------------------------------------------------------------------------------


-- ###############################################################
-- ## Origem: V1__schema.sql
-- ###############################################################
...
-- ###############################################################
-- ## Origem: V2__seed_veterinarios.sql
-- ###############################################################
...
-- ###############################################################
-- ## Origem: V3__seed_tutor_inicial.sql
-- ###############################################################
...
-- ###############################################################
-- ## Origem: V4__seed_pets_e_consultas.sql
-- ###############################################################
...
-- VALIDACAO DA CARGA
--------------------------------------------------------------------------------
SELECT p.NOME AS PET, p.ESPECIE, p.RACA, COUNT(c.ID_CONSULTA) AS TOTAL_CONSULTAS
FROM   PET p
LEFT JOIN CONSULTA c ON c.ID_PET = p.ID_PET
GROUP BY p.ID_PET, p.NOME, p.ESPECIE, p.RACA
ORDER BY p.ID_PET;
GO
